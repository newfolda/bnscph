-- Private storage and server-only API rate limiting.
...
create or replace function public.consume_api_rate_limit(p_endpoint text, p_key_hash text, p_limit integer, p_window_seconds integer)
returns boolean language plpgsql security definer set search_path = public as $$
declare next_count integer;
begin
  if p_endpoint !~ '^[a-z0-9-]{1,64}$' or length(p_key_hash) <> 64 or p_limit < 1 or p_window_seconds < 1 then return false; end if;
  insert into public.api_rate_limits (endpoint, key_hash, window_started_at, request_count) values (p_endpoint, p_key_hash, now(), 1)
